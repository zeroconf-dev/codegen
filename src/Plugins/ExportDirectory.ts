...
DO NOT EDIT DIRECTLY, CHANGES WILL BE DELETED UPON NEXT CODE GENERATION.`
	...
	let compiledSourceFile = ts.updateSourceFileNode(
		outputFile,
	...
	if (compiledSourceFile.statements.length === 0) {
		const compiledHeader = commentHeader.split('\n').map(comment => `// ${comment}`).join('\n');
		compiledSourceFile = ts.updateSourceFile(
			compiledSourceFile,
			compiledHeader,
		...
		commentHeader.split('\n').forEach(comment => {
			ts.addSyntheticLeadingComment(compiledSourceFile.statements[0], ts.SyntaxKind.SingleLineCommentTrivia, ` ${comment}`, true);
		});
	}
...
function createImportDeclarations(options: GenerateSingletonClassOptions, importExportMap: ImportExportMap) {
	return ts.createNodeArray(
		...
		].sort(({ importPath: a }, { importPath: b }) => a.localeCompare(b))
