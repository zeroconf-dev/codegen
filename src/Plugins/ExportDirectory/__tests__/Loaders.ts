import { plugin, ExportType } from '@zeroconf/codegen/Plugins/ExportDirectory';
import { runPlugins } from '@zeroconf/codegen/Runner';
...
const runPlugin = (pluginConfig: any, outputConfig?: Partial<OutputConfig>) =>
	runPlugins(
		createFilesystem(),
		{
			directory: '',
		...
		{
			debug: outputConfig?.debug ?? false,
			directory: outputConfig?.directory ?? '',
			globPattern: outputConfig?.globPattern ?? '',
			outputPattern: outputConfig?.outputPattern ?? '',
		},
		{
			ExportDirectory: {
				globalConfig: {
					outputPath: outputConfig?.outputPattern ?? '',
				},
...
const orgStream = fg.stream;
const mockInputFiles = (inputPaths: string[]) => {
	(fg.stream as any) = jest.fn().mockImplementation(() => ({
...
const orgCreateWriteStream = fs.createWriteStream;
const mockOutputStream = (outputStream: NodeJS.WritableStream) => {
	(fs.createWriteStream as any) = jest.fn().mockImplementation(() => outputStream);
};

beforeEach(() => {
	(fg.stream as any) = orgStream;
	(fs.createWriteStream as any) = orgCreateWriteStream;
});
...
describe('SingletonClass', () => {
	...
	test('constructorParameters', async () => {
		...
		await runPlugin({
			className: 'TestClass',
			constructorParameters: ['applicationName#string', 'startTime#number'],
			exportType: ExportType.SingletonClass,
			importPrefix: '../../',
			importTemplate: '${fileName}',
		});
